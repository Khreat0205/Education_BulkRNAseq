...
gctfh_files_path <- list.files(path = "1.Data/raw_count_txt/GCTFH/",full.names = T)
head(gctfh_files_path)

gctfh_files <- list.files(path = "1.Data/raw_count_txt/GCTFH/",full.names = F)
head(gctfh_files)
...
# * = 앞의 패턴 반복
# | = OR, 또는을 뜻함
gct_sample <- gsub(pattern = "^.*_D|.count.txt",
                   replacement = "",
                   x = gctfh_files)
## 1) ^.*_D = GSM4413011_D
...
gctfh_info <- function(x){
  ...
  sample_number <- substring(x,first=9,last=10)
  # substring("08GCTFH-01",first=9,last=10) # "01"
  sample_day <- substring(x,first=1,last=2)
  # substring("08GCTFH-01",first=1,last=2) # "08"
...
#### 추출된 그룹 정보를 테이블로 형성
head(gct_sample)
sample_to_info <- sapply(X = gct_sample,FUN = gctfh_info)
head(sample_to_info)
...
tfh_files_path <- list.files(path = "1.Data/raw_count_txt/TFHlike//",full.names=T) #
head(tfh_files_path)

tfh_files <- list.files(path = "1.Data/raw_count_txt/TFHlike//") #
head(tfh_files)
################  TO-DO ###################
# ex) GSM4413012_D08TFHlike-01.count.txt
# ^ = 개행 문자, 문자열의 시작점
# . = 모든 문자
# * = 앞의 패턴 반복
# | = OR, 또는을 뜻함
...
tfh_info <- function(x){
  ...
  # 1234567890ab
  ################  TO-DO ###################
  sample_number <- substring(x,
                            first=,
                            last=)
  sample_day <- substring(x,
                          first=,
                          last=)
  ################  TO-DO ###################
  result <- c(sample_celltype,sample_number,sample_day)
